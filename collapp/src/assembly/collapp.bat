...
rem  - HSQDLB: -Ddatasource.url=jdbc:hsqldb:mem:collapp
rem  - MYSQL: -Ddatasource.url=jdbc:mysql://localhost:3306/collapp?useUnicode=true&characterEncoding=utf-8
rem  - PGSQL: -Ddatasource.url=jdbc:postgresql://localhost:5432/collapp
rem
...
rem --cookiePrefix NAME - Set a prefix to the cookie name, so you can have multiple collapp instances on the same hostname
rem
rem example: java -jar collapp-jetty-console.war --port 8081 --bindAddress 127.0.0.1
rem
rem example: java -Dcollapp.config.location=file:/C:\... -jar collapp-jetty-console.war --port 8081 --bindAddress 127.0.0.1
...
java ^
	...
	-jar %~dp0/../collapp/collapp-jetty-console.war
