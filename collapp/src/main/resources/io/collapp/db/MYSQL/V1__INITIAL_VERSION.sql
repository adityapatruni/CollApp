...
CREATE TABLE LA_USER (
	...
	USER_LAST_EMAIL_SENT TIMESTAMP NULL DEFAULT NULL,
	USER_LAST_CHECKPOINT_COUNT INTEGER DEFAULT 0 NOT NULL,
	USER_LAST_CHECKED TIMESTAMP NULL DEFAULT NULL,
...
CREATE TABLE LA_BOARD (
	BOARD_ID INTEGER AUTO_INCREMENT PRIMARY KEY NOT NULL,
...
CREATE TABLE LA_BOARD_COLUMN_DEFINITION (
	BOARD_COLUMN_DEFINITION_ID INTEGER AUTO_INCREMENT PRIMARY KEY NOT NULL,
	BOARD_COLUMN_DEFINITION_PROJECT_ID_FK INTEGER NOT NULL,
	BOARD_COLUMN_DEFINITION_VALUE ENUM('OPEN', 'CLOSED', 'BACKLOG', 'DEFERRED') NOT NULL,
...
CREATE TABLE LA_BOARD_COLUMN (
	BOARD_COLUMN_ID INTEGER AUTO_INCREMENT PRIMARY KEY NOT NULL,
	BOARD_COLUMN_NAME VARCHAR(128) NOT NULL,
	BOARD_COLUMN_ORDER INTEGER NOT NULL,
	BOARD_COLUMN_LOCATION ENUM('BOARD', 'BACKLOG', 'ARCHIVE', 'TRASH') NOT NULL,
...
DELIMITER //
CREATE FUNCTION LA_FUN_BOARD_COLUMN_ENFORCE (BOARD_COLUMN_NAME VARCHAR(128), BOARD_COLUMN_LOCATION ENUM('BOARD', 'BACKLOG', 'ARCHIVE', 'TRASH'))
	RETURNS BOOLEAN
	DETERMINISTIC
	BEGIN
	RETURN BOARD_COLUMN_LOCATION IN ('BOARD', 'BACKLOG', 'ARCHIVE', 'TRASH') AND
		(CASE
...
CREATE TRIGGER CHECK_LA_BOARD_COLUMN_INSERT BEFORE INSERT ON LA_BOARD_COLUMN
	FOR EACH ROW
		BEGIN
			IF (NOT LA_FUN_BOARD_COLUMN_ENFORCE(NEW.BOARD_COLUMN_NAME, NEW.BOARD_COLUMN_LOCATION)) THEN
				CALL RAISE_CHECK_ERROR;
			END IF;
		END//
...
CREATE TABLE LA_BOARD_STATISTICS (
	...
	BOARD_STATISTICS_LOCATION ENUM('BOARD', 'BACKLOG', 'ARCHIVE', 'TRASH') NOT NULL,
...
CREATE TABLE LA_CARD (
	...
	CARD_LAST_UPDATED TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
	CARD_LAST_UPDATED_USER_ID_FK INTEGER NOT NULL
) ENGINE=InnoDB CHARACTER SET=utf8 COLLATE utf8_bin;


-- support table for FTS
CREATE TABLE LA_CARD_FTS_SUPPORT (
	CARD_FTS_SUPPORT_CARD_ID_FK INTEGER PRIMARY KEY NOT NULL,
	CARD_FTS_SUPPORT_CARD_NAME VARCHAR(255) NOT NULL,
	CARD_FTS_SUPPORT_LAST_UPDATED TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
	FULLTEXT(CARD_FTS_SUPPORT_CARD_NAME)
...
CREATE TABLE LA_CARD_DATA (
	CARD_DATA_ID INTEGER AUTO_INCREMENT PRIMARY KEY NOT NULL,
	CARD_DATA_CARD_ID_FK INTEGER NOT NULL,
	CARD_DATA_REFERENCE_ID INTEGER,
	CARD_DATA_DELETED BOOLEAN DEFAULT FALSE NOT NULL,
	CARD_DATA_TYPE ENUM('COMMENT', 'ACTION_LIST', 'ACTION_CHECKED', 'ACTION_UNCHECKED', 'FILE', 'COMMENT_HISTORY', 'DESCRIPTION', 'DESCRIPTION_HISTORY') NOT NULL,
	CARD_DATA_CONTENT MEDIUMTEXT NOT NULL,
	CARD_DATA_ORDER INTEGER NOT NULL,
	CARD_DATA_LAST_UPDATED TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB CHARACTER SET=utf8 COLLATE utf8_bin;
-- support table for FTS
CREATE TABLE LA_CARD_DATA_FTS_SUPPORT (
	CARD_DATA_FTS_SUPPORT_CARD_DATA_ID_FK INTEGER PRIMARY KEY NOT NULL,
	CARD_DATA_FTS_SUPPORT_CARD_DATA_CONTENT MEDIUMTEXT NOT NULL,
	CARD_DATA_FTS_SUPPORT_LAST_UPDATED TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
	FULLTEXT(CARD_DATA_FTS_SUPPORT_CARD_DATA_CONTENT)
) ENGINE=MyISAM CHARACTER SET=utf8 COLLATE utf8_general_ci;
...
DELIMITER //
CREATE FUNCTION LA_FUN_CARD_DATA_ENFORCE (CARD_DATA_TYPE ENUM('COMMENT', 'ACTION_LIST', 'ACTION_CHECKED', 'ACTION_UNCHECKED', 'FILE', 'COMMENT_HISTORY', 'DESCRIPTION', 'DESCRIPTION_HISTORY'), CARD_DATA_REFERENCE_ID INTEGER)
	RETURNS BOOLEAN
	DETERMINISTIC
	BEGIN
	RETURN CARD_DATA_TYPE IN ('COMMENT', 'ACTION_LIST', 'ACTION_CHECKED', 'ACTION_UNCHECKED', 'FILE', 'COMMENT_HISTORY', 'DESCRIPTION', 'DESCRIPTION_HISTORY') AND
		(CASE
...
CREATE TRIGGER CHECK_LA_CARD_DATA_INSERT BEFORE INSERT ON LA_CARD_DATA
	FOR EACH ROW
		BEGIN
			IF (NOT LA_FUN_CARD_DATA_ENFORCE(NEW.CARD_DATA_TYPE, NEW.CARD_DATA_REFERENCE_ID)) THEN
				CALL RAISE_CHECK_ERROR;
			END IF;
		END//

CREATE TRIGGER CHECK_LA_CARD_DATA_UPDATE BEFORE UPDATE ON LA_CARD_DATA
	FOR EACH ROW
		BEGIN
			IF (NOT LA_FUN_CARD_DATA_ENFORCE(NEW.CARD_DATA_TYPE, NEW.CARD_DATA_REFERENCE_ID)) THEN
				CALL RAISE_CHECK_ERROR;
			END IF;
			SET NEW.CARD_DATA_LAST_UPDATED = NOW();
		END//
...
CREATE TABLE LA_CARD_LABEL (
	CARD_LABEL_ID INTEGER AUTO_INCREMENT PRIMARY KEY NOT NULL,
	CARD_LABEL_PROJECT_ID_FK INTEGER NOT NULL,
	CARD_LABEL_UNIQUE BOOLEAN DEFAULT FALSE NOT NULL,
	CARD_LABEL_TYPE ENUM('NULL', 'STRING', 'TIMESTAMP', 'INT', 'CARD', 'USER', 'LIST') NOT NULL,
	CARD_LABEL_DOMAIN ENUM('SYSTEM', 'USER') NOT NULL,
...
CREATE FUNCTION LA_FUN_CARD_LABEL_ENFORCE (
	CARD_LABEL_TYPE ENUM('NULL', 'STRING', 'TIMESTAMP', 'INT', 'CARD', 'USER', 'LIST'),
	CARD_LABEL_DOMAIN ENUM('SYSTEM', 'USER'))
	RETURNS BOOLEAN
	DETERMINISTIC
	BEGIN
	RETURN CARD_LABEL_TYPE IN ('NULL', 'STRING', 'TIMESTAMP', 'INT', 'CARD', 'USER', 'LIST') AND CARD_LABEL_DOMAIN IN ('SYSTEM', 'USER');
	END//

CREATE TRIGGER CHECK_LA_CARD_LABEL_INSERT BEFORE INSERT ON LA_CARD_LABEL
	FOR EACH ROW
		BEGIN
			IF (NOT LA_FUN_CARD_LABEL_ENFORCE(NEW.CARD_LABEL_TYPE, NEW.CARD_LABEL_DOMAIN)) THEN
				CALL RAISE_CHECK_ERROR;
			END IF;
		END//

CREATE TRIGGER CHECK_LA_CARD_LABEL_UPDATE BEFORE UPDATE ON LA_CARD_LABEL
	FOR EACH ROW
		BEGIN
			IF (NOT LA_FUN_CARD_LABEL_ENFORCE(NEW.CARD_LABEL_TYPE, NEW.CARD_LABEL_DOMAIN)) THEN
				CALL RAISE_CHECK_ERROR;
...
CREATE TABLE LA_CARD_LABEL_LIST_VALUE (
	CARD_LABEL_LIST_VALUE_ID INTEGER AUTO_INCREMENT PRIMARY KEY NOT NULL,
...
-- junction table label <-> card
CREATE TABLE LA_CARD_LABEL_VALUE (
	CARD_LABEL_VALUE_ID INTEGER AUTO_INCREMENT PRIMARY KEY NOT NULL,
	...
	CARD_LABEL_ID_FK INTEGER NOT NULL,
	CARD_LABEL_VALUE_TYPE ENUM('NULL', 'STRING', 'TIMESTAMP', 'INT', 'CARD', 'USER', 'LIST') NOT NULL,
	CARD_LABEL_VALUE_STRING VARCHAR(32),
	CARD_LABEL_VALUE_TIMESTAMP TIMESTAMP NULL DEFAULT NULL,
...
CREATE TRIGGER CHECK_LA_CARD_LABEL_VALUE_INSERT BEFORE INSERT ON LA_CARD_LABEL_VALUE
	FOR EACH ROW
		BEGIN
			IF (NOT LA_FUN_CARD_LABEL_VALUE_ENFORCE(NEW.CARD_LABEL_VALUE_TYPE, NEW.CARD_LABEL_VALUE_STRING, NEW.CARD_LABEL_VALUE_TIMESTAMP, NEW.CARD_LABEL_VALUE_INT, NEW.CARD_LABEL_VALUE_CARD_FK, NEW.CARD_LABEL_VALUE_USER_FK, NEW.CARD_LABEL_VALUE_LIST_VALUE_FK)) THEN
				CALL RAISE_CHECK_ERROR;
			END IF;
		END//
...
CREATE TABLE LA_EVENT (
	...
	EVENT_TYPE ENUM('LABEL_CREATE', 'LABEL_DELETE', 'CARD_MOVE', 'CARD_CREATE', 'CARD_ARCHIVE', 'CARD_BACKLOG', 'CARD_TRASH', 'CARD_UPDATE', 'ACTION_LIST_CREATE', 'ACTION_LIST_DELETE',	'ACTION_ITEM_CREATE', 'ACTION_ITEM_DELETE', 'ACTION_ITEM_MOVE', 'ACTION_ITEM_CHECK', 'ACTION_ITEM_UNCHECK', 'COMMENT_CREATE', 'COMMENT_UPDATE', 'COMMENT_DELETE', 'FILE_UPLOAD', 'FILE_DELETE', 'DESCRIPTION_CREATE', 'DESCRIPTION_UPDATE') NOT NULL,
	...
	EVENT_LABEL_TYPE ENUM('NULL', 'STRING', 'TIMESTAMP', 'INT', 'CARD', 'USER') DEFAULT 'NULL' NOT NULL,
	EVENT_VALUE_INT INTEGER,
	EVENT_VALUE_STRING VARCHAR(255) NULL,
	EVENT_VALUE_TIMESTAMP TIMESTAMP NULL DEFAULT NULL,
	EVENT_VALUE_CARD_FK INTEGER NULL DEFAULT NULL,
	EVENT_VALUE_USER_FK INTEGER NULL DEFAULT NULL
) ENGINE=InnoDB CHARACTER SET=utf8 COLLATE utf8_bin;
-- CONSTRAINTS;
...
DELIMITER //
CREATE FUNCTION LA_FUN_EVENT_ENFORCE (EVENT_TYPE ENUM('LABEL_CREATE', 'LABEL_DELETE', 'CARD_MOVE', 'CARD_CREATE', 'CARD_ARCHIVE', 'CARD_BACKLOG', 'CARD_TRASH', 'CARD_UPDATE', 'ACTION_LIST_CREATE', 'ACTION_LIST_DELETE',	'ACTION_ITEM_CREATE', 'ACTION_ITEM_DELETE', 'ACTION_ITEM_MOVE', 'ACTION_ITEM_CHECK', 'ACTION_ITEM_UNCHECK', 'COMMENT_CREATE', 'COMMENT_UPDATE', 'COMMENT_DELETE', 'FILE_UPLOAD', 'FILE_DELETE', 'DESCRIPTION_CREATE', 'DESCRIPTION_UPDATE'))
	RETURNS BOOLEAN
	DETERMINISTIC
	BEGIN
	RETURN EVENT_TYPE IN ('LABEL_CREATE', 'LABEL_DELETE', 'CARD_MOVE', 'CARD_CREATE', 'CARD_ARCHIVE', 'CARD_BACKLOG', 'CARD_TRASH', 'CARD_UPDATE', 'ACTION_LIST_CREATE', 'ACTION_LIST_DELETE',	'ACTION_ITEM_CREATE', 'ACTION_ITEM_DELETE', 'ACTION_ITEM_MOVE', 'ACTION_ITEM_CHECK', 'ACTION_ITEM_UNCHECK', 'COMMENT_CREATE', 'COMMENT_UPDATE', 'COMMENT_DELETE', 'FILE_UPLOAD', 'FILE_DELETE', 'DESCRIPTION_CREATE', 'DESCRIPTION_UPDATE');
	END//

DELIMITER //
CREATE FUNCTION LA_FUN_EVENT_LABEL_TYPE_ENFORCE (EVENT_LABEL_TYPE ENUM('NULL', 'STRING', 'TIMESTAMP', 'INT', 'CARD', 'USER'))
	RETURNS BOOLEAN
	DETERMINISTIC
	BEGIN
	RETURN EVENT_LABEL_TYPE IN ('NULL', 'STRING', 'TIMESTAMP', 'INT', 'CARD', 'USER');
	END//

CREATE TRIGGER CHECK_LA_EVENT_INSERT BEFORE INSERT ON LA_EVENT
	FOR EACH ROW
		BEGIN
			IF (NOT LA_FUN_EVENT_ENFORCE(NEW.EVENT_TYPE)) THEN
				CALL RAISE_CHECK_ERROR;
			END IF;
			IF (NOT LA_FUN_EVENT_LABEL_TYPE_ENFORCE(NEW.EVENT_LABEL_TYPE)) THEN
				CALL RAISE_CHECK_ERROR;
			END IF;
		END//

CREATE TRIGGER CHECK_LA_EVENT_UPDATE BEFORE UPDATE ON LA_EVENT
	FOR EACH ROW
		BEGIN
			IF (NOT LA_FUN_EVENT_ENFORCE(NEW.EVENT_TYPE)) THEN
				CALL RAISE_CHECK_ERROR;
...
CREATE TABLE LA_PROJECT_ROLE (
	PROJECT_ROLE_ID INTEGER AUTO_INCREMENT PRIMARY KEY NOT NULL,
...
INSERT INTO LA_PROJECT_ROLE_PERMISSION(PROJECT_ROLE_ID_FK, PERMISSION) VALUES (LAST_INSERT_ID(), 'READ');
