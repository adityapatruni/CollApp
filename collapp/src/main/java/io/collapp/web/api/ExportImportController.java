...
import io.collapp.service.ImportService.TrelloBoardsResponse;
import io.collapp.service.ImportService.TrelloImportResponse;
...
public class ExportImportController {
	...
	public void export(HttpServletResponse resp) throws IOException {
		resp.setHeader("Content-Disposition", "attachment; filename=\"collapp-export-"
				+ new SimpleDateFormat("yyyy-MM-dd").format(new Date()) + ".zip\"");
		resp.setContentType("application/octet-stream");
		exportImportService.exportData(resp.getOutputStream());
	}

	@RequestMapping(value = "/api/import/collapp", method = RequestMethod.POST)
	@ResponseBody
	public void importFromcollapp(
