...
import static io.collapp.common.Constants.SYSTEM_LABEL_DUE_DATE;
...
public class ImportService {
	...
	public TrelloImportResponse importFromTrello(TrelloImportRequest importRequest, User user) {
		...
		Map<String, User> collappUsers = new HashMap<>();
		...
		for (BoardIdAndShortName boardIdAndShortName : importRequest.getBoards()) {
			...
			importBoard(trello, tImport, board, tBoard, collappUsers, importRequest.isImportArchived());
	...
	private void importBoard(Trello trello, TrelloImportResponse tImport, Board board, TrelloBoard tBoard,
			Map<String, User> collappUsers, boolean importArchived) {
			...
			for (Card iCard : list.getCards()) {
				...
				for (Action action : iCard.getActions(new Argument("filter", "commentCard"))) {
					tCard.comments.add(new TrelloComment(action.getDate(),
							getMemberFromId(collappUsers, boardMembers, action.getIdMemberCreator()),
							action.getData().getText()));
				}

				// Assigned users
				for (String memberId : iCard.getIdMembers()) {
					User user = getMemberFromId(collappUsers, boardMembers, memberId);
					if (user != null) {
	...
	private User getMemberFromId(Map<String, User> collappUsers, Map<String, Member> boardMembers, String memberId) {
		if (collappUsers.containsKey(memberId)) {
			return collappUsers.get(memberId);
		}
		Member member = boardMembers.get(memberId);
		User user = matchUser(collappUsers, memberId, member.getUsername());
		if (user != null)
			return user;

		user = matchUser(collappUsers, memberId, member.getEmail());
		if (user != null)
			return user;
		return matchUser(collappUsers, memberId, member.getFullName());
	}

	private User matchUser(Map<String, User> collappUsers, String memberId, String criteria) {
		if (StringUtils.isNotBlank(memberId)) {
			List<User> users = userRepository.findUsers(criteria);
			if (users.size() > 0) {
				collappUsers.put(memberId, users.get(0));
				return users.get(0);
			}
		}
		return null;
	...
	public void saveTrelloBoardsToDb(String projectShortName, TrelloImportResponse tImport, User user) {
		...
		for (String labelName : tImport.labels.keySet()) {
			if (!collappLabels.containsKey(labelName)) {
				...
				collappLabels.put(labelName, cl);
			}
		}
		...
		for (TrelloBoard tBoard : tImport.boards) {
			...
			for (Integer trelloColumnPos : tBoard.columns.keySet()) {
				...
				for (Integer pos : trelloColumn.cards.keySet()) {
					...
					importCard(card, boardId, boardColumnId, user, collappLabels, dueDateLabel, assignedLabel);
				}
			}
		}
	}
	...
	private void importCard(TrelloCard card, int boardId, int boardColumnId, User user,
			Map<String, CardLabel> collappLabels, CardLabel dueDateLabel, CardLabel assignedLabel) {
		...
		for (String labelName : card.labels) {
			labelService.addLabelValueToCard(collappLabels.get(labelName), cardId, new CardLabelValue.LabelValue(),
					user, new Date());
		}
	}
