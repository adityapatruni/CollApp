...
public class DataSourceConfig {
	...
	@Bean
    public collappEnvironment collappEnvironment(ConfigurableEnvironment environment) {
	    return new collappEnvironment(environment);
    }
	...
	public DataSource getDataSource(collappEnvironment env) throws URISyntaxException {
		...
		if (System.getProperty("startDBManager") != null) {
			...
			DatabaseManagerSwing.main(new String[] { "--url", "jdbc:hsqldb:mem:collapp", "--noexit" });
		}
	 ...
	 */
	private static void urlWithCredentials(HikariDataSource dataSource, collappEnvironment env)
			throws URISyntaxException {
	...
	private static void urlAndCredentials(HikariDataSource dataSource, collappEnvironment env) {
	...
	@Bean
	public DatabaseMigrator migrator(collappEnvironment env, DataSource dataSource, ApplicationEventPublisher publisher) {
