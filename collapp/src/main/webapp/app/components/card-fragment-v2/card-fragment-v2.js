(function () {
    ...
    angular.module('collapp.components').component('lvgCardFragmentV2', {
        bindings: {
    ...
    function createMainLink(targetState, projectName, boardShortName, sequenceNumber, isDynamicLink, cardIsClosed, $state, $location, subscribers, EventBus) {
        ...
        if (cardIsClosed) {
            a.className += ' collapp-closed-card';
        }
        if (isDynamicLink) {
    ...
    function updateCardClass(card, element) {
        if (card.columnDefinition !== 'CLOSED') {
            element.removeClass('collapp-closed-card');
        } else {
            element.addClass('collapp-closed-card');
        }
    }
